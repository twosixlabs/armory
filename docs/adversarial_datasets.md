...
\*\* The ground truth annotation for multi-object detecion is a 2D NDArray where each row represents a detection with format: `<timestep> <object_id> <bbox top-left x> <bbox top-left y> <bbox width> <bbox height> <confidence_score=1> <class_id> <visibility=1>`
