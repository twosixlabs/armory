...
The `gpus` flag is optional, and is ignored if `use_gpu` is false. If `use_gpu` is true, it defaults to using all GPUs.
    ...
    Setting the field to be `null` or setting the field to be `all` will enable use of all available gpus.
    In other words, `"gpus": null` or `"gpus": "all"` will enable all available GPUs.
