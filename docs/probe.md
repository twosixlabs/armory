# Probes and Meters: Examples for Adding to Code
...
We assume the user is modifying code for models or attacks and wishes to use `Probe`s and `Meter`s to monitor certain variables within the code.

Recall the steps for a minimal working example (in [Measurement Overview](./metrics.md)):
...
def user_init_function():
    meter = Meter(
        "my_layer_identity", lambda x: x, "my_model.layer_output"
    )
...
- The meter name (`"my_layer_identity"`) can be arbitrary within this context
...
...
```
This will prompt armory to run `user_init_function` in `user_init.py` before anything else is loaded for the scenario.

## Example 2: Attack Output
### User Story
...
- The meter name (`"my_attack_identity"`) can be arbitrary within this context
