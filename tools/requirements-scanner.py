...
class RequirementsScanner:
    ...
    def __repr__(self):
        requirements_dict = {
            f.name: content for (f, content) in self.requirements.items()
        }
        return json.dumps(requirements_dict, indent=2)

    def scan(self):
        files = [
            f for f in self.path.iterdir() if f.is_file() and f.name.endswith(".txt")
        ]
        requirements = {}
        for f in files:
                    ...
                    for package in needs_updating:
                        if line.startswith(package):
                            ...
                            if latest_version == self.ERROR_MSG:
                                errors.append(
                                    f"{package} in {f.name} not found on PyPI."
                                )
                            else:
                                print(
                                    f"Updating {package} in {f.name} to version {latest_version}..."
                                )
