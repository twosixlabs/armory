...
_URLS = "carla_video_tracking_test.tar.gz"
...
class CarlaVideoTrackingTest(tfds.core.GeneratorBasedBuilder):
  ...
  def _generate_examples(self, path):
    ...
    for video in videos:
      ...
      all_frames = glob.glob(os.path.join(path, video, "*.png")) # all images including RGB and semantic segmentation
      ss_frames = glob.glob(os.path.join(path, video, "*SS.png")) # all semantic segmentaiton frames
      ...
      def build_coords(mask):
        ...
        for thresh in np.arange(1, 0.05, -0.05):
          ...
          if corners is None:
            ...
            pts = corners.squeeze()
            break
        pts_by_dist = np.arctan2(pts[:,1]-pts[:,1].mean(),
                                    pts[:,0]-pts[:,0].mean())
