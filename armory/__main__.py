...
def configure(command_args, prog, description):
    ...
    if os.path.exists(default_host_paths.armory_config):
        ...
        if response in ("Y", "y", ""):
            ...
            config = load_global_config(default_host_paths.armory_config, validate=False)
    ...
    default_dataset_dir = config["dataset_dir"] if config is not None and "dataset_dir" in config.keys() else default_host_paths.dataset_dir
    default_local_dir = config["local_git_dir"] if config is not None and "local_git_dir" in config.keys() else default_host_paths.local_git_dir
    default_saved_model_dir = config["saved_model_dir"] if config is not None and "saved_model_dir" in config.keys() else default_host_paths.saved_model_dir
    default_tmp_dir = config["tmp_dir"] if config is not None and "tmp_dir" in config.keys() else default_host_paths.tmp_dir
    default_output_dir = config["output_dir"] if config is not None and "output_dir" in config.keys() else default_host_paths.output_dir
