...
class RandomFilterBaselineDefense(PoisonFilteringDefence):
    ...
    def detect_poison(self, **kwargs):
        ...
        if expected_pp_poison is None:
            ...
            log.info("Setting expected_pp_poison to 0.3.  This can be set under defense/kwargs in the config")
        
        if expected_pp_poison < 0 or expected_pp_poison > 1:
            raise ValueError(f"defense/kwargs/expected_pp_poison must be set between 0 and 1 in the config.  Got {expected_pp_poison}")


        is_clean = np.random.choice([0,1], self.n_data, p=[expected_pp_poison, 1 - expected_pp_poison])
