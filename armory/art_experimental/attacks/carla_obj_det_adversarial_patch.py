...
class CARLAAdversarialPatch(AdversarialPatchPyTorch):
    ...
    def _train_step(
        ...
        if self._optimizer_string == "pgd":
            ...
            if patch_grads.shape[0] == 6:
                patch_grads[3:,:,:] = torch.mean(patch_grads[3:,:,:], dim=0, keepdim=True)
                
            gradients = patch_grads.sign() * self.learning_rate
            if patch_grads.shape[0] == 6:
                gradients[3:,:,:] = gradients[3:,:,:] / self.learning_rate * self.learning_rate_depth
            ...
            raise ValueError('Adam optimizer for CARLA Adversarial Patch not supported.')
            self._optimizer.step()

            with torch.no_grad():
                if self._patch.shape[0] == 6:
                    self._patch[3:,:,:] = torch.mean(self._patch[3:,:,:], dim=0, keepdim=True)
                    self._patch[3:,:,:] = torch.clamp(
                        self._patch[3:,:,:], min=self.min_depth, max=self.max_depth
                    )    
    ...
    def _get_circular_patch_mask(self, nb_samples: int, sharpness: int = 40) -> "torch.Tensor":
        ...
        image_mask = np.ones((self.patch_shape[self.i_h_patch], self.patch_shape[self.i_w_patch]))
    ...
    def _random_overlay(
        ...
        for i_sample in range(nb_samples):
            ...
            endpoints = self.gs_coords[i_sample] # [topleft, topright, botright, botleft]
            enlarged_coords = np.copy(endpoints) # enlarge the green screen coordinates a bit to fully cover the screen
            ...
            image_mask_i = torchvision.transforms.functional.perspective(
                img=image_mask_i, startpoints=startpoints, endpoints=endpoints, interpolation=2, fill=0 #None
            )
            ...
            padded_patch_i = torchvision.transforms.functional.perspective(
                img=padded_patch_i, startpoints=startpoints, endpoints=endpoints, interpolation=2, fill=0 #None
            )
        ...
        foreground_mask = torch.all(torch.tensor(self.binarized_patch_mask == 0), dim=-1, keepdim=True).to(self.estimator.device)
        ...
        v_avg = 0.5647  # average V value (in HSV) for the green screen, which is #00903a
        ...
        if images.shape[1] == 6:      
            values_ratio_depth = torch.ones(values_ratio.size()).to(self.estimator.device) # no brightness adjustment for depth
        ...
        patched_images = torch.clamp(
            patched_images, min=self.estimator.clip_values[0], max=self.estimator.clip_values[1]
        )
    ...
    def generate(self, x, y=None, y_patch_metadata=None):
        ...
        for i in range(num_imgs):
            ...
            non_patch_idx = np.where(y_gt['labels'] != 4) # exclude the patch class, which doesn't exist in the training data
            ...
            if self.patch_shape[0] == 6: # initialize depth patch with average depth value of green screen
            ...
            patch, _ = super().generate(
                np.expand_dims(x[i], axis=0), y=[y_gt]
            )

            # Patch image
            x_tensor = torch.tensor(np.expand_dims(x[i], axis=0)).to(self.estimator.device)
            patched_image = self._random_overlay(images=x_tensor, patch=self._patch, scale=None, mask=None).detach().cpu().numpy()
            patched_image = np.squeeze(patched_image, axis=0)

            # Embed patch into background
            patched_image[
                np.all(self.binarized_patch_mask == 0, axis=-1)
            ] = x[i][np.all(self.binarized_patch_mask == 0, axis=-1)]

            patched_image = np.clip(patched_image, self.estimator.clip_values[0], self.estimator.clip_values[1])
