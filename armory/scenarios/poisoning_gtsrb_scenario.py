...
def poison_batch(src_imgs, src_lbls, src, tgt, batch_size, attack, fraction_poisoned):
    ...
    for idx in range(batch_size):
        if src_lbls[idx] == src and np.random.rand() < fraction_poisoned:
        ...
        if poison_dataset_flag:
            ...
            for x_test, y_test in tqdm(test_data, desc="Testing"):
                x_test, _ = poison_batch(
                    x_test, y_test, src_class, tgt_class, len(y_test), attack, 1.0
                )
