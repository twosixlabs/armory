...
class SleeperAgentScenario(Poison):
    ...
    def load_poisoner(self):
        ...
        if self.use_poison:
            ...
            K = sum(y_test == self.source_class)  # number of source-class test images to use in x_trigger
            ...
            attack_config["kwargs"]["indices_target"] = np.asarray(self.y_clean == self.target_class).nonzero()[0] # which train images belong to target class
            ...
            attack_config["kwargs"]["learning_rate_schedule"] = tuple(attack_config["kwargs"]["learning_rate_schedule"]) # convert to tuple as required by ART attack
            patch_size = attack_config["kwargs"].pop("patch_size")
            patch = Image.open( "armory/utils/triggers/" + attack_config["kwargs"]["patch"])
            ...
            x_trigger = copy.deepcopy(x_test[y_test == self.source_class])[:K]   # copy of test images to poison, because attack modifies these in place
            y_trigger = np.array([self.target_class] * K)               # target labels to classify test images into
