...
def asterisk(outdir) -> str:
    ...
    if not isinstance(outdir, BytesIO):
        outfile = os.path.join(os.path.abspath(outdir), f"{inspect.currentframe().f_code.co_name}.png")
    else:
        outfile = outdir
...
def grid(outdir, num_circles=25, circle_radius=0.5, circle_spacing=2.5) -> str:
    ...
    ax.set_xlim(0, circle_radius * 2 * num_circles + (circle_spacing - circle_radius * 2) * (num_circles - 1))
    ax.set_ylim(0, circle_radius * 2 * num_circles + (circle_spacing - circle_radius * 2) * (num_circles - 1))
...
class Shape:
    ...
    def from_name(cls, name) -> "Shape":
        ...
        if name not in _SHAPES:
            raise ValueError(f"Invalid shape name: {name}. Must be one of: {_SHAPES.keys()}")
