...
try:
    ...
    print("WARNING: armory.art_experimental.attacks.carla_obj_det_utils cannot be imported. Using stubbed functions.")
...
def rgb_depth_convert(command_args, prog, description):
    ...
    parser.add_argument(
        ...
        type=Path,
        nargs="+",
        help="Path to depth image to convert"
    )
    parser.add_argument(
        "-o",
        "--output",
        type=str,
        help="Path to save converted depth image"
    )
    parser.add_argument(
        "-s",
        "--show",
    ...
    if len(images) > 1:
        ...
        slider = widgets.Slider(ax_slider, 'Image', 0, len(images)-1, valinit=0, valstep=1)
